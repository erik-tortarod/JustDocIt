...
const ProgressBarModal = ({ isOpen, onClose, repoSize, language }: ProgressBarModalProps) => {
  ...
  const startProgressTracking = (size: number, lang: string) => {
    ...
    const milestones = [
      { percent: isSmallRepo ? 20 : 10, message: "Conectando con GitHub..." },
      { percent: isSmallRepo ? 40 : 25, message: "Analizando estructura del repositorio..." },
      { percent: isSmallRepo ? 70 : 50, message: `Procesando archivos de ${lang}...` },
      { percent: isSmallRepo ? 85 : 75, message: "Generando documentación..." },
    ...
    const updateProgress = () => {
      ...
      if (nextMilestone && currentProgress < 99) {
        ...
        if (milestones.some(m => m.percent === currentProgress)) {
          setStatusMessage(milestones.find(m => m.percent === currentProgress)!.message);
        }
        
        // Calcular siguiente intervalo - más lento cerca del final
        const nextInterval = currentProgress > 90 ? progressInterval * 1.5 : progressInterval;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-base-800 border border-base-700 rounded-lg shadow-xl w-full max-w-md p-6">
        <div className="text-center mb-4">
          <h3 className="text-lg font-semibold text-content-100 mb-1">
        ...
        <div className="mb-2 flex justify-between items-center">
          <span className="text-sm text-content-300">{statusMessage}</span>
          <span className="text-sm font-semibold text-content-100">{progress}%</span>
        </div>
        
        <div className="w-full bg-base-900 rounded-full h-2.5 mb-4">
          <div 
            className={`h-2.5 rounded-full transition-all duration-300 ease-in-out ${
              isCompleted ? "bg-success-500" : "bg-accent-400"
            }`}
        ...
        {isCompleted ? (
          <div className="mt-4 p-3 bg-success-900 border border-success-700 text-success-300 rounded-md text-sm">
          ...
          <div className="flex justify-end mt-4">
            <button 
              onClick={onClose}
              className="px-4 py-2 text-sm bg-base-700 text-content-200 rounded-md hover:bg-base-600 transition-colors"
            >
              Cancelar
            </button>
          </div>
